...
- Explicit Break Handler and login-recovery pauses freeze the active Builder task and its commitment clock rather than replacing its goal/strategy/time display or allowing planner recovery to run. Ordinary `super.run()` skipped ticks keep the wall-clock commitment timer advancing; the overlay still distinguishes generic pauses, antiban cooldowns, and antiban breaks while preserving the active plan until an explicit external pause releases.
...
- **Commitment-clock correction (2026-09-11):** A generic Microbot `super.run()` skip no longer enters the Builder's external-pause path. Task commitment time and task action deadlines now continue to use real elapsed time through skipped worker ticks; only explicit Break Handler ownership and login stabilization pause those clocks.
- **Child-task completion audit (2026-09-11):** Every Builder parent task was checked for nested-task terminal-status leakage. Cooking was the sole mismatch: it propagated its successful banking child's `COMPLETE` after loading raw food, so the planner abandoned cooking before walking to the range. It now advances to `WALK_TO_COOKING` and returns `RUNNING`, matching combat, supply, quests, and the other skilling tasks.
- **Firemaking trip variation (2026-09-11):** Each firemaking task now selects one of five nearby open-air Varrock West campfire spots when it starts. The selected spot stays fixed for that trip, while nearby existing fires and Forester's Campfires are still preferred, avoiding both repetitive placement and unstable mid-task rerouting.
- **Transient task retry (2026-09-11):** A bounded task-level failure no longer immediately sends the account to an unrelated goal. After `TRAVEL_FAILED`, `RESOURCE_NOT_FOUND`, `ACTION_FAILED`, or `PRODUCTION_WIDGET_FAILED`, Builder recreates the active task once with fresh action guards and the same commitment clock. A second failure, missing supply/tool, level gate, bank failure, or manual skip still follows the normal replan path.
- **Al Kharid Warriors (2026-09-11):** Added the F2P Al Kharid Palace warrior cluster as a mid-level combat method. It is available from combat 20 through the level-40 recommendation window, targets the verified palace spawn center at `(3295, 3170)`, accepts both observed NPC-name spellings, and requires a ten-item cooked-food loadout (reduced by the normal combat food policy at higher combat levels). The shared combat bank task withdraws that food together with the selected weapon and any owned armour.
...
- **Compact overlay cleanup (2026-09-11):** Builder now uses one fixed 230-pixel status panel rather than compact and detailed modes. It shows only state, goal, current need, task phase, remaining time, and the five-column skill icon grid; redundant activity, score, mode, stop-reason, and detail-toggle UI were removed.
...
- [x] Live Sheep Shearer completion audit: starting the quest and turning in wool are separate Fred conversations. The task now uses the exact start dialogue only while `NOT_STARTED`, then re-talks to Fred and uses only the exact wool turn-in dialogue while `IN_PROGRESS`. An in-progress quest receives a completion-priority score; a `FINISHED` quest is rejected by both its goal and strategy and cannot create a task.
- [x] Live Sheep Shearer partial-hand-in audit: Fred accepts balls one at a time; live varp `179` reached `20`, meaning nineteen had been accepted and one remained. The Builder now reads the cached Sheep varplayer through `AccountContext`, mirrors the bundled Quest Helper's remaining-ball formula, and withdraws/requests only the outstanding amount until the quest state is `FINISHED`.
- [x] Simplify combat food preparation: chickens remain the no-food fresh-account bootstrap, while every other combat activity requires at least ten cooked-food items in the bank. A qualifying combat loadout deposits its inventory, then withdraws every available cooked-food stack with the bank's native `Withdraw-All` action after gear is selected; a smaller reserve makes combat ineligible instead of creating a food-supply detour.
- [x] Harden Builder's logout/world-transition guard: a task now yields whenever the client lacks a logged-in game state, top-level world view, local player, or readable player location. This closes the Break Handler race where the login flag remained true while the world view had already been cleared, which previously surfaced as a `WorldView.getBaseX()` null pointer in the Need field.
